...
typedef pcl::PointXYZI PointCloudType;    											// 一个激光点  更倾向于命名为PointType
typedef pcl::PointCloud<pcl::PointXYZI> PointCloud;       			  // 激光点云
typedef pcl::PointCloud<pcl::PointXYZI>::Ptr PointCloudPtr;	   // 激光点云指针
typedef std::vector<std::vector<int>> ClusterIndices;

// 定义了一个类，存分割算法的配置参数
struct SegmentationParameter {
    bool ground_removal;    // 是否去地面
...
std::vector<pcl::PointIndices> region_growing(
    ...
    sor.filter(*cloud);       // todo：它这个不是只读的吗？为什么还能进行修改
