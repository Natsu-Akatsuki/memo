...
# 步骤： * 假定 `MSG_DEP_SET` 是一系列要使用相关消息类型的包 e.g. std_msgs, actionlib_msgs, ... * 1.
# 在package.xml中： * 1.1 增加build_depend tag for "message_generation" * 1.2
# 增加build_depend and a exec_depend tag for each package in MSG_DEP_SET * If
# MSG_DEP_SET isn't empty the following dependency has been pulled in but can be
# declared for certainty nonetheless: * 增设exec_depend tag for "message_runtime"
# * 2. 在CMakeLists.txt中： * 2.1 add "message_generation" and every package in
# MSG_DEP_SET to find_package(catkin REQUIRED COMPONENTS ...) * 2.2 add
...
# 生成自定义的消息类型（build阶段，生成在build/下） add_message_files( FILES   # (argument)
...
# 根据依赖，生成msg源文件（run阶段，生成在devel/下） generate_messages( DEPENDENCIES std_msgs  # Or
...
# projects also need catkin_package( #  INCLUDE_DIRS include #  LIBRARIES
# pysript #  CATKIN_DEPENDS other_catkin_pkg #  DEPENDS system_lib )

# ##############################################################################
# 编译 ##
# ##############################################################################

# 指定额外的头文件搜索目录 Your package locations should be listed before other locations
include_directories(# include ${catkin_INCLUDE_DIRS}
)

# 构建库 add_library(${PROJECT_NAME} src/${PROJECT_NAME}/pysript.cpp )
...
# catkin_add_gtest(${PROJECT_NAME}-test test/test_pysript.cpp) if(TARGET
