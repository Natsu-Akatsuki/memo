...
def print_help():
    ...
    print()
    print('Converts image sequence(s) in ros bag file(s) to video file(s) with fixed frame rate using',
...
class RosVideoWriter():
    def __init__(self, fps=25.0, rate=1.0, topic="", output_filename="", display=False, verbose=False,
                 start=rospy.Time(0), end=rospy.Time(sys.maxsize)):
        ...
        while self.t_video[topic] < self.t_file[topic] / self.rate:
            if not topic in self.p_avconv:
                ...
                if video_fmt == MJPEG_VIDEO:
                    cmd = [VIDEO_CONVERTER_TO_USE, '-v', '1', '-stats', '-r', str(self.fps), '-c', 'mjpeg', '-f',
                           'mjpeg', '-i', '-', '-an', out_file]
                    ...
                    cmd = [VIDEO_CONVERTER_TO_USE, '-v', '1', '-stats', '-r', str(self.fps), '-f', 'rawvideo', '-s',
                           size, '-pix_fmt', pix_fmt, '-i', '-', '-an', out_file]
                    self.p_avconv[topic] = subprocess.Popen(cmd, stdin=subprocess.PIPE)
                    if self.opt_verbose:
                        print("Using command line:")
                        print(cmd)
        ...
        for topic, msg, t in bag.read_messages(connection_filter=self.filter_image_msgs, start_time=self.opt_start,
            ...
            try:
                if msg.format.find("jpeg") != -1:
                    if msg.format.find("8") != -1 and (
                            msg.format.find("rgb") != -1 or msg.format.find("bgr") != -1 or msg.format.find(
                            "bgra") != -1):
                        if self.opt_display_images:
