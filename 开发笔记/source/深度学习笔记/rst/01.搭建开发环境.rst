...
.. note:: 在ubuntu16中执行时或返回空值，则可以使用`官方安装包 <https://www.nvidia.cn/Download/index.aspx?lang=cn>`_ 进行安装
...
* 
  ``nouveau`` （开源，但功能非常少）和 ``nvidia driver`` 都是nvidia的显卡驱动。部分计算机默认使用 ``nouveau`` 作为驱动，那么在这些机子上装N卡官网驱动时，就有冲突的问题，\ `需要先关闭  <https://docs.nvidia.com/cuda/cuda-installation-guide-linux/index.html#runfile-nouveau>`_\ ``nouveau``\ ` 模块 <https://docs.nvidia.com/cuda/cuda-installation-guide-linux/index.html#runfile-nouveau>`_\ （记得update）。
...
步骤一：\ `tar包下载与安装 <https://developer.nvidia.com/nvidia-tensorrt-download>`_
